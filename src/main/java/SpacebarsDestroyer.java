...
/**
 * Client and main class for SpacebarsDestroyer.
 *
 * @author Stanislav Rakitov
 */
public class SpacebarsDestroyer {
  ...
  public static void main(String[] args) throws IOException {
    // Start server thread
    Thread serverThread = new Thread(new SpacebarsDestroyerServer(SERVER_ADDRESS, SERVER_PORT));
    serverThread.start();
    ...
    // Поскольку у нас тестовая задачка держать работающий тред с сервером смысла нет. Хотя можно
    // коннектиться телнетом и наслаждаться ответами сервера.
